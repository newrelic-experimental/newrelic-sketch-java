...
// A logical array with an index window. Window can start anywhere in "long" range, including negative numbers.
// Window start and end is auto updated on array write. Write will fail when window size exceeds maxSize
...
public class WindowedCounterArray {
    ...
    private long indexBase;  // Logical index of entry 0 in backingArray. Must be within [indexStart, indexEnd]
    ...
    public long get(final long index) {
        if (index < indexStart || index > indexEnd) {
            throw new IndexOutOfBoundsException("index=" + index + " indexStart=" + indexStart + " indexEnd=" + indexEnd);
        }
